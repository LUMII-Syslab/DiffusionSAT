...
def tsp_unsupervised_loss(predictions, adjacency_matrix, noise=0, log_in_tb=False, fast_inaccurate=False, subtour_projection=False):
    ...
    if fast_inaccurate or False:  # todo šis ar padding
        ...
        if subtours_cy:
            ...
            cut_weight = tf.sparse.sparse_dense_matmul(subtours_sparse, predictions)  # All these cut_weight values are < 1
            cost_subtours += tf.reduce_sum(tf.square(1 - cut_weight)) / tf.cast(batch_size, tf.float32)

            if subtour_projection:
                # add constraint cut_weight >= 1.
                dif = (1 - cut_weight) / tf.sparse.reduce_sum(subtours_sparse, axis=1, keepdims=True)  # how much each prediction should be increased
