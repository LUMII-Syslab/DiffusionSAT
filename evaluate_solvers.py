...
def test_walksat():
    ...
    solved = []
    time_used = []
    for step_data in itertools.islice(dataset.test_data(), 10):
    ...
    cum_solved = numpy.cumsum(solved)
    cum_time = numpy.cumsum(time_used)
    rows = [[sol, tim] for sol, tim in zip(cum_solved, cum_time)]
